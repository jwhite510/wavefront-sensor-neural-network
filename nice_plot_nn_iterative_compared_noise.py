...
if __name__ == "__main__":
    ...
    ax.pcolormesh(f,f,files['out_0']['retrieved_nn']['measured_pattern'],cmap='jet')
    ax.xaxis.set_ticks([])
    ax.set_ylabel(r"frequency [1/m]$\cdot 10^{6}$")
    for i,retrieval in zip([0,1],['retrieved_nn','retrieved_iterative']):
        ax=fig.add_subplot(gs[i,2])
        complex_obj=np.squeeze(files['out_0'][retrieval]['real_output'] + 1j*files['out_0'][retrieval]['imag_output'])
        ...
        ax.text(0.05,0.95,'Neural Network\nRetrieved Intensity',transform=ax.transAxes,color='white',weight='bold',va='top')
        ...
        if not i==0: ax.yaxis.set_ticks([])
        else:ax.set_ylabel(("position [um]"))
        ax.xaxis.set_ticks([])
        ax.text(0.05,0.95,'Neural Network\nRetrieved Phase',transform=ax.transAxes,color='black',weight='bold',va='top')
        place_colorbar(im,ax,offsetx=0.015,offsety=0.005,ticks=[-3.14,0,3.14],color='black',labels=['-pi','0','+pi'])

    fig.savefig('./iterative_nn_compared_noise.png')
