...
if __name__ == "__main__":
    ...
    for _i,_retrieval in enumerate(['intensity','phase']):
        ...
        ax.errorbar(np.array([snr(float(pc)) for pc in peakcounts],dtype=float),network_avg,network_std,label='neural network',alpha=0.5,color='blue',linewidth=4.0)
        ax.errorbar(np.array([snr(float(pc)) for pc in peakcounts],dtype=float),iterative_avg,iterative_std,label='iterative',alpha=0.5,color='orange',linewidth=4.0)
