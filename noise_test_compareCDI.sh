...
wfsc_arr=(0 2)
noisecountv_arr=(0 50)
for wfsc in ${wfsc_arr[@]}
do
	for noisecountv in ${noisecountv_arr[@]}
	do
		echo "running test:""wfsc:"$wfsc"	noisecountv:"$noisecountv
		for k in $(seq 1 14)
		do
			...
			pc="$noisecountv"
			wfsensor="$wfsc" # wavefront sensor
