...
mpirun -np 20 a.out --count 200 --name ${network}_train.hdf5 --buffersize 100 --seed 8977
cd ../..

rm -rf ./$DIR
cd zernike3/build/
python addnoise.py --infile ${network}_test.hdf5 --outfile ${network}_test_noise.hdf5 --peakcount 50 --cameraimage $camera_noise
python addnoise.py --infile ${network}_train.hdf5 --outfile ${network}_train_noise.hdf5 --peakcount 50 --cameraimage $camera_noise
cd ../..
